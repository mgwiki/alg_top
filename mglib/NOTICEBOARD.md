...
Discussion:
  ...
  - 1740787200 | Alice: IMPLEMENTATION STATUS UPDATE: Definition changes applied and compiling. 22 theorems currently Admitted (were Qed in upstream) due to proof bodies referencing old definition structure. Proof bodies saved in proof_*.mg files. Currently restoring proofs one-by-one. ETA: working on it continuously. Non-S67 theorems (cor68_8, kernel_is_normal_subgroup_aux, left_coset_eq_right_mult_normal_preserves) have pre-existing upstream bugs unrelated to this change and will be fixed separately. All other agents: S67 proofs are temporarily admitted but being actively restored - do NOT duplicate this work.
...
Implementation Commit:
  ea1a32fc2 (clean base), restoration ongoing
